...
**Reaction Quotient (\(Q\))**:
- Same form as \(K_c\), but uses current concentrations (not equilibrium)
- Compare \(Q\) to \(K_c\) to predict direction:
  - \(Q < K_c\): reaction shifts right (toward products)
  - \(Q > K_c\): reaction shifts left (toward reactants)
...
- **Concentration change**: Adding reactant/product shifts to consume it
- **Pressure change** (gases): Increasing pressure shifts toward fewer moles of gas
- **Temperature change**:
  - Exothermic reaction (\(\Delta H < 0\)): Increasing T shifts toward reactants
  - Endothermic reaction (\(\Delta H > 0\)): Increasing T shifts toward products
- **Catalyst**: Speeds up both forward and reverse rates equally; no shift in position
...
### Example 2: Calculating Equilibrium Constant
...
**Solution**:
1. Write \(K_c\) expression: \(K_c = \frac{[HI]^2}{[H_2][I_2]}\)
2. Substitute equilibrium concentrations:
3. \(K_c = \frac{(1.2)^2}{(0.20)(0.20)} = \frac{1.44}{0.040} = 36\)
...
??? note "Click to reveal answers"
    1. At equilibrium, the **forward and reverse reaction rates are equal**, so concentrations remain constant. Concentrations are **not necessarily equal** to each other, just constant.

    2. \(K_c = \frac{[SO_3]^2}{[SO_2]^2[O_2]}\)

    3. **Products favored**. \(K_c > 1\) means the numerator (products) is larger than denominator (reactants).

    4. \(K_c = \frac{[PCl_3][Cl_2]}{[PCl_5]}\). Rearrange: \([PCl_3] = \frac{K_c \cdot [PCl_5]}{[Cl_2]} = \frac{(0.050)(0.10)}{0.15} = \frac{0.005}{0.15} = \mathbf{0.033 \text{ M}}\)

    5. \(Q = \frac{[NO_2]^2}{[N_2O_4]} = \frac{(0.20)^2}{0.50} = \frac{0.04}{0.50} = 0.08\). Since \(Q (0.08) < K_c (0.10)\), the reaction **shifts right** toward products.

    6. (a) Endothermic means heat is a reactant. Increasing T shifts **right**, favoring products. (b) Removing \(CO_2\) (product) shifts **right** to replace it.

    7. **No effect** on equilibrium position. A catalyst speeds up both forward and reverse reactions equally, so equilibrium is reached faster but the position doesn't change.

    8. Color will **lighten**. Cooling an exothermic reaction shifts right toward \(N_2O_4\) (colorless), consuming brown \(NO_2\).

    9. **Reactants favored** (\(K_c < 1\)). If \(K_c\) increases with temperature, the reaction **shifts right** when heated, meaning it's **endothermic** (heat favors products).

    10. **No effect**. Both sides have equal moles of gas (1 + 1 = 2 on left, 2 on right), so pressure changes don't shift equilibrium.
